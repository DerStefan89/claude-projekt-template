---
description: Lessons Learned der Session fuer die eigene Wissensbasis aufbereiten
---

Fasse diese Session als Lernmaterial zusammen. Halte dich strikt an dieses
Format. Keine Einleitung, kein Nachwort.

## Was gebaut oder geaendert wurde
Stichpunkte mit Dateipfaden. Nur, was nachweislich auf der Platte gelandet ist.

## Was schiefgelaufen ist
Pro Punkt: Symptom, Ursache, Fix. Auch eigene Denkfehler benennen.

## Welches Anti-Pattern sichtbar wurde
Name und wie es sich zeigte. Wenn keins auftrat: "keins".

## Was davon in eine dauerhafte Regel gehoert
Pro Punkt: welche Datei (CLAUDE.md / ARCHITECTURE.md / state/gates.md /
ein Skill), welcher Abschnitt, und der fertige Textbaustein zum Einfuegen.

[FUELLUNG] Falls dieses Projekt an einer eigenen Playbook-Bibliothek
lernt (wie toolkompass an claude-playbook): hier die Liste der eigenen
Kapitel eintragen, damit die Auswahl nicht frei, sondern gebunden ist -
eine Auswahlliste verhindert erfundene Kategorien.

## Offene Punkte fuer die naechste Session
Nummerierte Liste, jeweils mit dem konkreten naechsten Handgriff.
